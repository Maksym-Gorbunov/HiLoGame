<template>
  <tr :class="{editable: this.editable == true}" :id="item._id">
    ...
    <td>
      <button @click="edit()" type="button" class="btn btn-primary btn-sm my_btn">edit</button>
      <button @click="deleteItem()" type="button" class="btn btn-danger btn-sm my_btn">delete</button>
    </td>
  </tr>
</template>
  ...
  methods: {
    edit() {
      if (!this.editable) {
        ...
        tr.childNodes[3].childNodes[1].classList.add('disabled')
        tr_question = tr.childNodes[0].innerHTML
        tr_difficulty = tr.childNodes[1].innerHTML
        tr_answer = tr.childNodes[2].innerText
        ...
        new_tr.childNodes[3].childNodes[1].classList.remove('disabled')
        let new_question = new_tr.childNodes[0].innerHTML
        let new_difficulty = new_tr.childNodes[1].innerHTML
        let new_answer = new_tr.childNodes[2].innerText
